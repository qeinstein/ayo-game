...
export const metadata: Metadata = {
  title: 'How Ayò Ọlọ́pọ́n is built — Engineering notes',
  description:
    'The architecture, concurrency model, scalability and engineering trade-offs behind the Ayò Ọlọ́pọ́n game service.',
};
...
function Tier({ label, title, lines, tone }: { label: string; title: string; lines: string[]; tone: string }) {
  return (
    <div className="card p-5">
      ...
      <ul className="mt-2 space-y-1 text-[12.5px] text-neutral-400">
        {lines.map((l) => (
          <li key={l}>{l}</li>
        ))}
      </ul>
...
function Card({
  ...
  return (
    <div className="card p-6">
      ...
      <div className="space-y-3 text-[13.5px] leading-relaxed text-neutral-400">{children}</div>
    </div>
  );
}

const Code = ({ children }: { children: React.ReactNode }) => (
  <code className="rounded bg-black/50 px-1.5 py-0.5 font-mono text-[12px] text-wood-brassSoft">{children}</code>
);
    ...
    <div className="flex min-h-screen flex-col">
      <header className="sticky top-0 z-40 border-b border-line/70 bg-ink/70 backdrop-blur-xl">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-3.5 sm:px-6">
          <Link href="/" className="flex items-center gap-2 text-sm font-medium text-neutral-400 transition-colors hover:text-neutral-100">
            <ArrowLeft className="h-4 w-4" /> Back to the board
          </Link>
          <span className="text-[11px] uppercase tracking-[0.2em] text-neutral-600">Engineering notes</span>
        </div>
      </header>
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-10 sm:px-6 sm:py-14">
        ...
        <div className="max-w-2xl animate-slide-up">
          ...
          <p className="mt-4 text-[15px] leading-relaxed text-neutral-400">
            Ayò Ọlọ́pọ́n is a full-stack game service: a <span className="text-neutral-200">Spring Boot 3 / Java 17</span>{' '}
            REST backend that runs the authentic rules and a minimax AI, and this <span className="text-neutral-200">Next.js</span>{' '}
        ...
        <section className="mt-12">
          <h2 className="mb-4 text-[11px] font-medium uppercase tracking-[0.2em] text-neutral-500">Architecture</h2>
        ...
        <section className="mt-12 grid gap-5 md:grid-cols-2">
          <Card icon={<Boxes className="h-4 w-4" />} title="What was built">
            <p>
              A stateful, multi-session game server enforcing the real Yoruba ruleset: counter-clockwise sowing, the
            ...
            <p>
              ...
              <span className="text-neutral-200">minimax with alpha-beta pruning to depth 4</span>. The client mirrors the
          ...
          <Card icon={<Cpu className="h-4 w-4" />} title="Concurrency model">
            ...
            <p>
              ...
              <span className="text-neutral-200"> sow → capture → score → turn-flip → AI reply</span> commits as one
          ...
          <Card icon={<Layers className="h-4 w-4" />} title="How it scales">
            <p>
              ...
              singleton, so <span className="text-neutral-200">every move across every game serializes</span> through one
          ...
          <Card icon={<TriangleAlert className="h-4 w-4" />} title="Problems encountered">
            <p>
              The subtle rules bit hardest: getting <span className="text-neutral-200">grand-slam</span> and{' '}
              <span className="text-neutral-200">anti-starvation feeding</span> to agree with the capture walk took the
        ...
        <section className="mt-5">
          <div className="card flex flex-col gap-4 p-6 sm:flex-row sm:items-center">
            ...
            <div className="flex-1">
              <h3 className="text-sm font-semibold text-neutral-100">The AI, briefly</h3>
            ...
            <Link
              href="/"
              className="shrink-0 rounded-lg bg-wood-brass px-4 py-2 text-center text-[13px] font-semibold text-black transition-colors hover:bg-wood-brassSoft"
            >
              Play a match
            </Link>
          </div>
        ...
        <p className="mt-10 flex items-center gap-2 text-[11px] text-neutral-600">
